...
const componentHooks = /* @__PURE__ */ new WeakMap();
...
function hooks() {
  ...
  if (!componentHooks.has(instance)) componentHooks.set(instance, /* @__PURE__ */ new Map());
  return componentHooks.get(instance);
}
function useState(initialValue) {
  const values = hooks();
  const key = hookKey("state");
  if (!values.has(key)) values.set(key, /* @__PURE__ */ ref(typeof initialValue === "function" ? initialValue() : initialValue));
...
function useRef(initialValue) {
  ...
  if (!values.has(key)) {
    const value = /* @__PURE__ */ ref(initialValue);
    Object.defineProperty(value, "current", { get: () => value.value, set: (next) => {
      value.value = next;
    } });
...
function useMemo(factory, dependencies) {
  ...
  if (!previous || !sameDeps(previous.dependencies, dependencies)) {
    const next = { dependencies: dependencies == null ? void 0 : dependencies.slice(), value: factory() };
    values.set(key, next);
    return next.value;
  }
...
function useEffect(effect, dependencies) {
  ...
  const current = { dependencies: dependencies == null ? void 0 : dependencies.slice(), cleanup: previous == null ? void 0 : previous.cleanup };
...
const SUPABASE_URL = "https://gmddnvtkthajwodyingl.supabase.co";
const PUBLISHABLE_KEY = "sb_publishable_-DsUAg3oW-D3hEHtgeKb4A_sqwEPVmy";
...
const adminLogin = async (email, password) => {
  ...
  if (!response.ok || !result.access_token) throw new Error(result.error_description || "登入失敗");
  return { token: result.access_token };
};
const adminPost = async (action, payload = {}) => {
...
const createPublicCase = async (values) => (await request("publicCreate", values)).caseNo;
const preparePublicUploads = async (photos) => request("publicPrepareUploads", { photos });
const uploadSignedPhoto = async (signedUrl, file, mimeType) => {
...
const queryCase = async (caseNo, phone) => ({ case: (await request("query", { caseNo, phone })).case });
const workerGet = async (action, parameters = {}) => {
  const result = await request("workerList", parameters);
...
const prepareWorkerUploads = async (payload) => request("workerPrepareUploads", payload);
const workerPost = async (action, payload = {}) => request("workerComplete", { ...payload, caseId: payload.id, files: payload.files || [] });
const toCasePayload = (item) => item;
if ("serviceWorker" in navigator) {
  {
    window.addEventListener("load", () => {
      navigator.serviceWorker.register(`${"/miaolibulkywastevue/"}sw.js`).catch((error) => {
...
export {
  ...
  isVNode as i,
  createApp as j,
  createPublicCase as k,
  prepareWorkerUploads as l,
  workerPost as m,
  preparePublicUploads as p,
  queryCase as q,
  toCasePayload as t,
  useState as u,
  workerGet as w

...
const selectableTables = new Set(['cases', 'case_history', 'photo_sync_jobs'])
...
export default function DatabaseViewer({ cases, getMinguoTime }) {
  ...
  const rowKey = (item) => String(table === 'cases' ? item.case_no : table === 'system_settings' ? item.setting_key : item.id)
  const visibleKeys = records.map(rowKey).filter(Boolean)
  const allVisibleSelected = Boolean(visibleKeys.length) && visibleKeys.every((key) => selectedRows.includes(key))
  useEffect(() => { setSelectedRows([]) }, [table])
  useEffect(() => {
  ...
  const toggleAll = () => setSelectedRows((current) => allVisibleSelected ? current.filter((key) => !visibleKeys.includes(key)) : [...new Set([...current, ...visibleKeys])])
  const removeSelected = async () => {
    if (!selectedRows.length || !window.confirm(`確定刪除已選取的 ${selectedRows.length} 筆資料嗎？此動作無法復原。`)) return
    setLoading(true)
    const deleted = []
    try {
      for (const keyValue of selectedRows) { await adminPost('databaseDelete', { table, keyValue }); deleted.push(keyValue) }
      setDatabase((old) => ({ ...old, [table]: (old[table] || []).filter((row) => !deleted.includes(rowKey(row))) }))
      setSelectedRows([])
    } catch (error) {
      if (deleted.length) setDatabase((old) => ({ ...old, [table]: (old[table] || []).filter((row) => !deleted.includes(rowKey(row))) }))
      setSelectedRows((current) => current.filter((key) => !deleted.includes(key)))
      window.alert(`批次刪除中斷：已刪除 ${deleted.length} 筆。${error.message}`)
    } finally { setLoading(false) }
  }
  return <main className="mx-auto max-w-[1600px] p-4 sm:p-6"><header className="mb-5"><p className="text-sm font-black text-emerald-700">資料庫檢視</p><h2 className="mt-1 text-2xl font-black">Supabase 資料庫</h2><p className="mt-2 text-sm text-slate-500">可切換所有系統資料表；案件資料可自訂欄位、時間與照片。</p></header><nav className="mb-5 flex flex-wrap gap-2">{tables.map(([key,label]) => <button key={key} onClick={() => setTable(key)} className={'rounded-xl px-4 py-2.5 text-sm font-black '+(table===key?'bg-emerald-700 text-white':'bg-white text-slate-600 shadow-sm')}>{label}{table===key&&loading?'（讀取中）':''}</button>)}</nav><section className="rounded-2xl bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:justify-between"><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold text-slate-600">搜尋資料<input value={keyword} onInput={(e)=>setKeyword(e.target.value)} className="mt-1 w-full rounded-xl border px-4 py-2.5 font-normal"/></label>{table==='cases'&&<label className="text-sm font-bold text-slate-600">時間區隔<select value={period} onChange={(e)=>setPeriod(e.target.value)} className="mt-1 w-full rounded-xl border px-4 py-2.5 font-normal"><option value="all">全部時間</option><option value="year">今年度</option><option value="quarter">本季</option><option value="month">本月</option><option value="custom">自訂區間</option></select></label>}{table==='cases'&&period==='custom'&&<div className="sm:col-span-2 grid grid-cols-2 gap-3"><label>開始日期<input type="date" value={dateFrom} onInput={(e)=>setDateFrom(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2"/></label><label>結束日期<input type="date" value={dateTo} onInput={(e)=>setDateTo(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2"/></label></div>}</div>{table==='cases'&&<div className="lg:max-w-3xl"><p className="mb-2 text-sm font-black">顯示項目（已選欄位可用 ← → 調整左右順序）</p><div className="flex flex-wrap gap-2">{caseColumns.map(([key,label]) => <span key={key} className={'inline-flex items-center rounded-full border text-xs font-bold '+(selected.includes(key)?'border-emerald-600 bg-emerald-700 text-white':'border-slate-300 bg-white text-slate-600')}><label className="cursor-pointer px-3 py-1.5"><input type="checkbox" checked={selected.includes(key)} onChange={()=>toggle(key)} className="sr-only"/>{label}</label>{selected.includes(key)&&<><button onClick={()=>move(key,-1)} className="border-l border-white/30 px-2 py-1.5">←</button><button onClick={()=>move(key,1)} className="border-l border-white/30 px-2 py-1.5">→</button></>}</span>)}</div></div>}</div></section><section className="mt-5 overflow-hidden rounded-2xl bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4"><h3 className="font-black">{tables.find(([key])=>key===table)?.[1]}</h3><div className="flex flex-wrap items-center gap-2"><span className="mr-1 text-sm font-bold text-slate-500">{records.length} 筆{selectedRows.length ? `・已選 ${selectedRows.length} 筆` : ''}</span>{selectableTables.has(table)&&<><button disabled={!records.length || loading} onClick={toggleAll} className="rounded-xl border border-emerald-600 px-4 py-2 text-sm font-black text-emerald-700 disabled:opacity-40">{allVisibleSelected ? '取消全選' : '全選'}</button><button disabled={!selectedRows.length || loading} onClick={removeSelected} className="rounded-xl bg-rose-600 px-4 py-2 text-sm font-black text-white disabled:opacity-40">刪除</button><button disabled={!selectedRows.length || loading} onClick={()=>setSelectedRows([])} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-black text-slate-600 disabled:opacity-40">取消</button></>}<button disabled={!columns.length} onClick={exportCsv} className="rounded-xl bg-emerald-700 px-4 py-2 text-sm font-black text-white disabled:opacity-40">⇩ 匯出 CSV</button></div></div>{columns.length?<div className="max-h-[65vh] overflow-auto"><table className="min-w-max w-full text-left text-sm"><thead className="sticky top-0 bg-slate-100 text-xs text-slate-600"><tr><th className="px-3 py-3">{selectableTables.has(table)?<input type="checkbox" checked={allVisibleSelected} onChange={toggleAll} aria-label="全選目前列表" className="h-4 w-4 accent-emerald-700"/>:'刪除'}</th>{columns.map(([key,label])=><th key={key} className="whitespace-nowrap px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y">{records.map((item,index)=>{const keyValue=rowKey(item),checked=selectedRows.includes(keyValue);return <tr key={item.case_no||item.id||index} className={checked?'bg-emerald-50':'hover:bg-emerald-50'}><td className="px-3 py-3">{selectableTables.has(table)?<input type="checkbox" checked={checked} onChange={()=>toggleRow(keyValue)} aria-label={`選取 ${keyValue}`} className="h-4 w-4 accent-emerald-700"/>:<button onClick={()=>remove(item)} className="rounded-lg border border-rose-200 px-2 py-1 text-xs font-black text-rose-700">刪除</button>}</td>{columns.map(([key])=><td key={key} className="max-w-xs whitespace-nowrap px-4 py-3">{key==='case_no'?<strong className="text-emerald-700">{value(item,key)}</strong>:<span className="block max-w-xs truncate" title={String(value(item,key))}>{value(item,key)}</span>}</td>)}</tr>})}{!records.length&&<tr><td colSpan={columns.length+1} className="py-14 text-center font-bold text-slate-400">沒有符合的資料</td></tr>}</tbody></table></div>:<p className="p-12 text-center font-bold text-slate-400">目前沒有資料</p>}</section></main>
}
